...
async function uj_info(){
   console.log("megnyomódtam");
    let data = await getch("https://starwars-databank-server.onrender.com/api/v1/characters");
    console.log(data);
    console.log("adat megérkezett és már ki is írtam");
    let randomelem = veletlen_szam(0,9)
    faj.innerHTML = data.data[randomelem].name;
    leir.innerHTML = data.data[randomelem].description;
    kep.src = data.data[randomelem].image;
}
